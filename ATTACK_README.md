...
Build the malicious contract, publish it through a real stacks-node, and watch the signer's `/new_block` handler 413 the resulting block.

Prerequisites: sBTC devenv running (`make devenv-up`), plus a locally-built `stacks-cli` from a sibling `stacks-network/stacks-core` checkout.
...
The observer on :8811 dumps the same block body to `/tmp/poc_block_*.raw.json` so its size can be measured directly.
