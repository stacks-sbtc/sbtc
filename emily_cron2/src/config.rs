//! Runtime configuration and command-line options.

use clap::Parser;

use crate::logging::LogOutputFormat;

/// Runtime endpoints, reconciliation thresholds, and scheduling options.
#[derive(Parser)]
...
pub struct Config {
    ...
    /// Additional Bitcoin blocks required before expiry or replacement updates.
    #[arg(long, env = "MIN_BLOCK_CONFIRMATIONS", default_value_t = 6)]
    pub min_block_confirmations: u64,
    /// Maximum age in seconds for a missing pending transaction.
    #[arg(long, env = "MAX_UNCONFIRMED_TIME", default_value_t = 86400)]
    pub max_unconfirmed_time: u64,
    /// Run once and exit (suitable for an external scheduler).
    #[arg(long)]
    pub once: bool,
    /// Log proposed updates without writing to Emily.
    #[arg(long)]
    pub dry_run: bool,
    /// Delay in seconds between completed reconciliation cycles.
