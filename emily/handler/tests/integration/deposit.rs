...
#[tokio::test]
async fn emily_return_404_if_all_deposits_in_update_request_are_unknown() {
    // the testing configuration has privileged access to all endpoints.
    let testing_configuration = clean_setup().await;

    // Create two deposits, but not sending them to Emily.
    let bitcoin_tx_output_index = 0;
    ...
    let err = apis::deposit_api::update_deposits_signer(
        &testing_configuration,
        update_deposits_request_body,
    )
    .await
    .expect_err("Update deposits request where all deposits are unknown should return 404");
    let status = match err {
        testing_emily_client::apis::Error::ResponseError(ResponseContent { status, .. }) => status,
        e => panic!("Expected a 404 error, got {:#?}", e),
    };
