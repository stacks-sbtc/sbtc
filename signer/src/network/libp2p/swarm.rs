...
///
/// The derive macro composes each behaviour's
/// `handle_established_*_connection` call in declaration order and bailing
/// on a returned `Err`, so gatekeepers must appear before any behaviour
/// that records per-connection state. `allow_signers` runs first to reject
/// non-signer peers during the upgrade phase; `connection_limits` then
/// enforces numeric slot limits; `bootstrap` follows after other gates
/// have approved.
#[derive(NetworkBehaviour)]
pub struct SignerBehavior {
    pub allow_signers: allow_signers::Behavior,
...
impl SignerBehavior {
    pub fn new(keypair: Keypair, config: SignerSwarmConfig) -> Result<Self, SignerSwarmError> {
        ...
        Ok(Self {
            allow_signers: allow_signers::Behavior::new(config.signer_state),
