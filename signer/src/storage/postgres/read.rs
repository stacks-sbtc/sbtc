...
use crate::{
    DEPOSIT_LOCKTIME_BLOCK_BUFFER, WITHDRAWAL_BLOCKS_EXPIRY,
    bitcoin::{
        utxo::SignerUtxo,
        validation::{
...
impl PgRead {
    ...
    where
        E: sqlx::PgExecutor<'e, Database = sqlx::Postgres> + Send,
    {
...
impl DbRead for PgStore {
    async fn get_bitcoin_block(
        ...
        PgRead::get_bitcoin_block(self.pool(), block_hash).await
    }
    ...
    async fn get_stacks_block(
        ...
        PgRead::get_stacks_block(self.pool(), block_hash).await
    }

    async fn get_bitcoin_canonical_chain_tip(
        &self,
    ) -> Result<Option<model::BitcoinBlockHash>, Error> {
        PgRead::get_bitcoin_canonical_chain_tip(self.pool()).await
    }
