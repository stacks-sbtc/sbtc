...
async fn libp2p_peer_storage() {
    ...
    let mut term1 = context1.get_termination_handle();
    let mut term2 = context2.get_termination_handle();
    ...
    println!("--- STARTING SWARMS (ROUND 1) ---");
    ...
    // Get a copy of each context's stored p2p peers for assertions
    ...
    // Verify that context 1 has the peer from context 2. We'll only verify
    // context 1's peers since context2 uses the same code.
    ...
    // Wait for the swarms to shut down
    term1.wait_for_shutdown().await;
    term2.wait_for_shutdown().await;
    let _ = tokio::join!(handle1, handle2);
    tokio::time::sleep(tokio::time::Duration::from_millis(1000)).await;

    println!("--- STARTING SWARMS (ROUND 2) ---");
    ...
    // Start the two swarms again. This time, they should know about each other
    // and attempt to connect at startup.
    let mut swarm1_clone = swarm1.clone();
    let context1_clone = context1.clone();
    ...
    let p2p_peers_1b = context1.get_storage().get_p2p_peers().await.unwrap();
    let p2p_peers_2b = context2.get_storage().get_p2p_peers().await.unwrap();
    assert_eq!(p2p_peers_1b.len(), 1);
    assert_eq!(p2p_peers_2b.len(), 1);
    assert!(p2p_peers_1b[0].last_dialed_at > p2p_peers_1a[0].last_dialed_at);
    assert!(p2p_peers_2b[0].last_dialed_at > p2p_peers_2a[0].last_dialed_at);
}
