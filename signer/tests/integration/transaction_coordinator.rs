...
/// Test that we run DKG if signer set changed
#[test(tokio::test)]
async fn run_dkg_signer_set_changed() {
    let mut rng = get_rng();
    ...
    // 3.5 Temporarly remove last signer to add it later
    let new_signers = signers.clone();
    let new_signer = signers.pop().unwrap();
    for signer in &mut signers {
        let mut signer_set_pubkeys = signer_set_public_keys.clone();
        signer_set_pubkeys.remove(&new_signer.2.public_key().into());
        signer
            .0
            .state()
            .update_current_signer_set(signer_set_pubkeys);
    }
    ...
    // Now let's add new signer to signer set and perform DKG again

    let signers = new_signers;
    ...
    // Await the `stacks_tx_receiver_task` to receive the first transaction broadcasted.
    let broadcast_stacks_txs: StacksTransaction =
        tokio::time::timeout(Duration::from_secs(10), stacks_tx_receiver_task)
            .await
            .unwrap()
            .expect("failed to receive message")
