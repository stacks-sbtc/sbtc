...
async fn mock_stacks_core<D, B, E>(
    ...
    ctx.with_stacks_client(|client| {
        ...
        client.expect_get_tenure_headers().returning(move |consensus_hash| {
            ...
            // should be so.
            let last_sortition_ch_bytes = (*consensus_hash).into_bytes().map(|byte| byte.overflowing_add(1).0);
            tenure.last_sortition_ch = signer::storage::model::ConsensusHash::new(last_sortition_ch_bytes);
...
async fn sign_bitcoin_transaction() {
    ...
    wait_for_tenure_completed(&signers, block_hash).await;
...
async fn process_rejected_withdrawal(is_completed: bool, is_in_mempool: bool) {
    ...
    let tenure_completed_signal = TxCoordinatorEvent::TenureCompleted(bitcoin_chain_tip).into();
    context
